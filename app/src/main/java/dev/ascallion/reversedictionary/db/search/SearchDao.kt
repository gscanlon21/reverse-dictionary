...
interface SearchDao {
    ...
    fun insertMany(items: List<SearchResultEntity>)

    @Query("SELECT * FROM searchresult WHERE type = :type AND `key` = :key")
    fun getList(type: ApiType, key: String): List<SearchResultEntity>

    @Query("SELECT * FROM searchresult WHERE type = :type AND `key` IN (:keys)")
    fun getList(type: ApiType, keys: List<String>): List<SearchResultEntity>

    @Insert(onConflict = OnConflictStrategy.IGNORE)
    fun insertWordOfTheDay(item: WordOfTheDayEntity)

    /**
     * Retrieves the most recent saved [WordOfTheDayEntity]
     */
    @Query("SELECT * FROM wotd ORDER BY createdTime DESC LIMIT 1")
    fun getWordOfTheDay(): WordOfTheDayEntity
}
