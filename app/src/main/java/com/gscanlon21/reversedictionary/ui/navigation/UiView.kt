...
sealed class UiView(val name: String, val resId: Int) {
     ...
     * - [com.gscanlon21.reversedictionary.repository.data.ApiType] to handle the API
     * - [R.array.search_result_preference] to handle the user preference settings
     *
     * @param sortOrder
     * @param name
     * A unique string value used to map to the API
     * @param resId
     * A unique int value, when converted to its string value it is used to map to the User Preference 'Pages to Show'
     */
    sealed class SearchResult(val sortOrder: Int, name: String, resId: Int) : UiView(name, resId) {
        ...
        companion object {
            ...
            /**
             * Used to map res/values/strings/search_result_preference to the SearchResult type
             * @param name String
            ...
            fun resValueOf(name: String, context: Context): SearchResult {
                return values()
                    .single { context.getString(it.resId) == name }
            }
